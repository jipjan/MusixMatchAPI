...
using TrackGet = MusixMatch_API.APIMethods.Track.TrackGet;
using TrackLyricsGet = MusixMatch_API.APIMethods.Track.TrackLyricsGet;
using TrackSearch = MusixMatch_API.APIMethods.Track.TrackSearch;
using TrackSnippetGet = MusixMatch_API.APIMethods.Track.TrackSnippetGet;
using TrackSubtitleGet = MusixMatch_API.APIMethods.Track.TrackSubtitleGet;
...
namespace MusixMatch_API
{
    ...
    public class MusixMatchApi : IDisposable
    {
        ...
        public void ChartArtistsGet(ArtistsGet get, Action<List<ReturnedArtists.ArtistList>> succes, Action<string> fail)
        {
        ...
        /// <param name="fail">Action to run when the call failed, returns an error.</param>
        public void TrackSubtitleGet(TrackSubtitleGet get, Action<ReturnedSubtitle.Subtitle> succes, Action<string> fail)
        {
        ...
        /// <summary>
        /// This API method provides you the opportunity to help us improving our catalogue. We aim to provide you with the best quality service imaginable, so we are especially interested in your detailed feedback to help us to continually improve it.
        /// Please take all the necessary precautions to avoid users or automatic software to use your website/app to use this commands, a captcha solution like http://www.google.com/recaptcha or an equivalent one has to be implemented in every user interaction that ends in a POST operation on the musixmatch api.
        /// </summary>
        /// <param name="get">The LyricsFeedbackPost object which contains the filters to post the feedback for the correct lyrics and song.</param>
        /// <param name="succes">When the call to the API succeeds, returns the uploaded feedback (or nothing, this is not shown on the musixmatch doc page...)</param>
        /// <param name="fail">Action to run when the call failed, returns an error.</param>
        ...
        /// <param name="succes">When the call to the API succeeds, return the Subtitle object.</param>
        /// <param name="fail">Action to run when the call failed, returns an error.</param>
        public void MatcherSubtitleGet(MatcherSubtitleGet get, Action<ReturnedSubtitle.Subtitle> succes, Action<string> fail)
        {
        ...
        /// <param name="fail">Action to run when the call failed, returns an error.</param>
        public void ArtistAlbumsGet(ArtistAlbumsGet get, Action<List<ReturnedAlbums.AlbumList>> succes, Action<string> fail)
        {
        ...
        /// <param name="succes">When the call to the API succeeds, return the AlbumList object with albums.</param>
        /// <param name="fail">Action to run when the call failed, returns an error.</param>
        public void ArtistRelatedGet(ArtistRelatedGet get, Action<List<ReturnedArtists.ArtistList>> succes, Action<string> fail)
        {
            RunApiCall<ReturnedArtists.RootObject>(get, item => succes(item.Message.Body.ArtistList), fail);
        }
        ...
        {
            RunApiCall<ReturnedCatalogue.RootObject>(new CatalogueDumpGet(), item => succes(item.Message.Body.Catalogue), fail);
        }
